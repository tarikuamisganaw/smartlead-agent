...
<<<<<<< HEAD
  const googleMissing =
    status.lead_sync_provider === "google_sheets" && !status.lead_sync_configured;

  return (
    <div className="grid gap-4 md:grid-cols-2">
      <section className="rounded-md border border-line bg-white p-4 shadow-sm">
        <h2 className="text-sm font-semibold text-ink">Lead Sync</h2>
        <dl className="mt-3 grid gap-3 text-sm">
          <Info label="Provider" value={formatLabel(status.lead_sync_provider)} />
          <div>
            <dt className="text-xs font-semibold uppercase text-ink/45">Configured</dt>
            <dd className="mt-1"><StatusBadge value={status.lead_sync_configured} /></dd>
          </div>
        </dl>
        {googleMissing ? (
          <p className="mt-3 rounded-md border border-gold/30 bg-gold/10 px-3 py-2 text-sm text-gold">
            Google Sheets provider is selected but missing credentials or spreadsheet ID.
          </p>
        ) : null}
      </section>
      ...
      <section className="rounded-md border border-line bg-white p-4 shadow-sm">
        ...
        <dl className="mt-3 grid gap-3 text-sm">
          <Info label="Provider" value={formatLabel(status.notification_provider)} />
          <div>
            <dt className="text-xs font-semibold uppercase text-ink/45">Configured</dt>
            <dd className="mt-1"><StatusBadge value={status.notification_configured} /></dd>
          </div>
        </dl>
=======
  return (
